                ...
                <div className='details' >
                    ...
                    <h1 className='heading' > {token?.name} </h1>{' '}
                        <div className='location' >
                            ...
                            <div className='stats' >
                            <div className='col-4' >
                                <h4 > 20 </h4> <p> Reviews </p > {' '} </div>{' '} <div className='col-4' >
                                <h4 > 10 </h4> <p> Bookings </p > {' '}        </div>{' '} <div className='col-4' >
                                <h4 > 5 </h4> <p> 5 - Star </p > {' '} </div>{' '} </div>{' '} <div className='stat2' >
                            <div className='col-12' >
                                <button className='btn btn-dark bck'
                                    onClick={e => goBackToRoutes(e)} >
                                    GO BACK {' '} </button>{' '} </div>{' '} </div>{' '} </div>{' '} </header>{' '} </section>{' '} </div>
    )
}